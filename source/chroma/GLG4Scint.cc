...
GLG4Scint::GLG4Scint(const G4String& tablename, G4double lowerMassLimit) {
    ...
    std::cout << "GLG4Scint: " << tablename << " added to list of scintillation processes." << std::endl;
...
GLG4Scint::PostPostStepDoIt(const G4Track& aTrack, const G4Step& aStep) {
    ...
    //std::cout << "GLG4Scint::PostPostStepDoIt 2" << std::endl;
    ...
    //std::cout << "GLG4Scint::PostPostStepDoIt 3" << std::endl;
    ...
    //std::cout << "GLG4Scint::DoIt() " << numSecondaries << " secondaries ... fin" << std::endl;
...
G4VParticleChange * GLG4Scint::GenericPostPostStepDoIt(const G4Step *pStep) {
    ...
    for (size_t i = 0; i < masterVectorOfGLG4Scint.size(); i++) {
        if (mass > masterVectorOfGLG4Scint[i]->myLowerMassLimit) {
            //std::cout << "GLG4Scint::GenericPostPostStepDoIt() " << i << " " << masterVectorOfGLG4Scint[i]->myLowerMassLimit << " " << mass << std::endl;
            return masterVectorOfGLG4Scint[i]->PostPostStepDoIt(*track, *pStep);
        }
    }
    //std::cout << "GLG4Scint::GenericPostPostStepDoIt() no match" << std::endl;
    return NULL;
}
